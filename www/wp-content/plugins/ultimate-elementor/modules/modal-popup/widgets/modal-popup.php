...
class Modal_Popup extends Common_Widget {
		...
		if ( parent::is_internal_links() ) {
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s doc link */
					'raw'             => sprintf( __( '%1$s Close Modal Popup on click of button or link » %2$s', 'uael' ), '<a href="https://uaelementor.com/docs/is-it-possible-to-close-a-modal-popup-on-the-click-of-a-button-or-text/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">', '</a>' ),
					'content_classes' => 'uael-editor-doc',
				]
			);
		...
		switch ( $content_type ) {
			...
			case 'photo':
				if ( isset( $dynamic_settings['ct_photo']['url'] ) ) {
					return '<img src="' . $dynamic_settings['ct_photo']['url'] . '" />';
				}
				return '<img src="" />';
			break;
	...
	public function get_video_embed( $settings, $node_id ) {
		...
		if ( 'youtube' == $settings['content_type'] ) {
			...
			$html .= '<div class="uael-modal-iframe uael-video-player" data-src="youtube" data-id="' . $vid_id . '" data-append="?version=3&enablejsapi=1' . $related_videos . $title_controls . $player_controls . '" data-thumb="' . $thumb . '"></div>';
			...
			if ( '' != $vid_id && 0 != $vid_id ) {
				...
				$html .= '<div class="uael-modal-iframe uael-video-player" data-src="vimeo" data-id="' . $vid_id . '" data-append="?title=0&byline=0&portrait=0&badge=0" data-thumb="' . $thumb . '"></div>';
			}
		}
		$html .= '</div>';
		return $html;
