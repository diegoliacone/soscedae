...
// Table.
$this->add_render_attribute( 'uael_table_id', 'id', 'uael-table-id' );
if ( 'responsive' !== $settings['responsive'] ) {
	$this->add_render_attribute( 'uael_table_id', 'class', 'uael-text-break' );
}
$this->add_render_attribute( 'uael_table_id', 'class', 'column-rules' );
...
?>
<div <?php echo $this->get_render_attribute_string( 'uael_table_wrapper' ); ?>>
	<div class="uael-advance-heading" >
		<div class="uael-tbl-entry-wrapper">
			<?php if ( 'yes' === $settings['show_entries'] ) { ?>
				<?php $row_count = 1; ?>
				<label class="uael-lbl-entry"><?php _e( 'Show Entries: ', 'uael' ); ?></label><select id="uael-show-entries">
