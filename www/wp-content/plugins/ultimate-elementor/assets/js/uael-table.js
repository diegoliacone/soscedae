( function( $ ) {
	...
	var WidgetUAELTableHandler = function( $scope, $ ) {
		...
		var uael_table_id        = $scope.find( '#uael-table-id' );
		...
		function sortTable(n) {
			...
			while ( switching ) {
				...
				for ( i = 0; i < ( rows.length - 1); i++ ) {
					...
					x = rows[i].getElementsByClassName("uael-table-col")[n];
					y = rows[i + 1].getElementsByClassName("uael-table-col")[n];
		...
		function coloumn_rules() {
			if($(window).width() > 767) {
				$(uael_table).addClass('column-rules');
				$(uael_table).removeClass('no-column-rules');
			}else{
				$(uael_table).removeClass('column-rules');
				$(uael_table).addClass('no-column-rules');
			}
		}
