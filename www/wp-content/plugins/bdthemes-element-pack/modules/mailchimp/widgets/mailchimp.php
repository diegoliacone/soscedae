...
class Mailchimp extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .bdt-newsletter-wrapper .bdt-newsletter-input-wrapper' => 'width: 100%;',
				],
			]
		);
	...
	public function render() {
		...
		<div class="bdt-newsletter-wrapper">
			...
			<form action="<?php echo site_url() ?>/wp-admin/admin-ajax.php" id="bdt-mailchimp-<?php echo esc_attr($id); ?>" class="bdt-grid<?php echo esc_attr($space); ?> bdt-child-width-auto" bdt-grid>
				<div class="bdt-newsletter-input-wrapper">
					<input type="email" name="email" placeholder="<?php echo esc_attr($settings['email_field_placeholder']); ?>" required class="bdt-input bdt-width-1-1" />
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				"use strict";
				$('#bdt-mailchimp-<?php echo esc_attr($id); ?>').submit(function(){
					var mailchimpform = $(this);
					bdtUIkit.notification({message: '<div bdt-spinner></div> <?php esc_html_e( 'Subscribing you please wait...', 'bdthemes-element-pack'); ?>', timeout: false, status: 'primary'});
					$.ajax({
