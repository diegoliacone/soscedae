...
class Post_List extends Widget_Base {
	...
	public function render() {
		...
		$id       = $this->get_id();
		$classes  = ['bdt-post-list', 'bdt-post-list-skin-base'];
		
		$this->add_render_attribute('list-wrapper', 'class', ['bdt-post-list-item', 'list-part'] );

		if ( 'yes' == $settings['show_horizontal'] ) {
			$this->add_render_attribute('list-wrapper', 'bdt-grid', '' );
			$this->add_render_attribute('list-wrapper', 'bdt-grid', 'bdt-grid' );
			$this->add_render_attribute('list-wrapper', 'class', ['bdt-child-width-1-' . $settings['column'] . '@m'] );
		} else {
		...
		if( $wp_query->have_posts() ) :
			...
			?> 
			<div id="bdt-post-list-<?php echo esc_attr($id); ?>" class="<?php echo \element_pack_helper::acssc($classes); ?>">
